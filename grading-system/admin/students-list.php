...
$sql = "SELECT id, user_id, surname, first_name, middle_name, role, section FROM users WHERE TRIM(LOWER(role)) = 'student'";
...
<head>
    ...
    function updateAllStudents() {
        ...
        rows.forEach(row => {
            ...
            data.push({ id, user_id, surname, first_name, middle_name, role, section });
        });

        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'update_user.php', true); // Use the correct PHP handler
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.onload = function () {
            alert(xhr.status == 200 ? xhr.responseText : "Update failed!\n" + xhr.responseText);
        };
            ...
            <?php if ($result && $result->num_rows > 0): ?>
                <table>
                    ...
                    <?php endwhile; ?>
                            ...
                            <button class="update-btn" onclick="updateAllStudents()">Save All</button>
